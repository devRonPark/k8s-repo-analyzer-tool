...
| `spring_boot.py` | — | Spring Boot / Gradle rules. Synthesizes the app component: Java toolchain, Spring Boot version + starters (WebFlux→reactive/Netty vs servlet/Tomcat), gradlew + bootJar/bootRun/test, executable-JAR artifact (`build/libs/<name>-<version>.jar`), default 8080, Actuator liveness/readiness (+`/livez`,`/readyz` only when `add-additional-paths` enabled), `bootBuildImage` without a Dockerfile, app-needs-no-PVC, Spring-specific `unresolved`. DB-related facts (H2 default, `SPRING_PROFILES_ACTIVE`, `spring.sql.init`, `production_database_selection`) are emitted **only when `_has_db_evidence`** holds (a JDBC driver, JPA/JDBC starter, datasource/`database` property, or non-demo SQL init) — never invented for a DB-less app; the image-name example uses the real project name. Pure; one-way. |
