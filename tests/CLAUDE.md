...
| `test_spring_boot_generalization.py` | — | Category regression on a synthetic (non-fixture) Gradle Spring Boot repo: name/port/env derived from content, server.port override, `/livez`,`/readyz` only when enabled, no-actuator case, PVC not required |
